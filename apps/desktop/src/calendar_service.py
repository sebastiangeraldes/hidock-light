...
class MeetingCorrelator:
    ...
    def _find_best_meeting_match(
        self, recording_time: datetime, meetings: List[MeetingInfo]
    ) -> Tuple[Optional[MeetingInfo], float]:
        """
    ...
    def get_correlation_statistics(self, correlations: List[RecordingCorrelation]) -> Dict[str, Any]:
        ...
        avg_confidence = sum(c.correlation_confidence for c in correlations if c.meeting_info) / max(
            1, correlated + manual
        )
