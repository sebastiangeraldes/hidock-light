...
"""
Status Filter Widget for HiDock Light
...
class StatusFilterWidget(ctk.CTkFrame):
    ...
    def _create_widgets(self):
        ...
        # Filter label
        self.filter_label = ctk.CTkLabel(self, text="📁 Filter by Status:", font=ctk.CTkFont(size=12, weight="bold"))
        self.filter_label.grid(row=0, column=0, padx=(5, 10), pady=5, sticky="w")

        # Status dropdown
        self.status_dropdown = ctk.CTkOptionMenu(
            self, width=160, height=32, values=list(self.FILTER_OPTIONS.values()), command=self._on_filter_changed
        )
        ...
        self.status_info_label = ctk.CTkLabel(self, text="", font=ctk.CTkFont(size=10), text_color="gray")
    ...
    def _on_filter_changed(self, selected_display_name: str):
        ...
        try:
            ...
            if filter_key is None:
                logger.warning(
                    "StatusFilter", "filter_changed", f"Unknown filter display name: {selected_display_name}"
                )
                return
    ...
    def get_current_filter(self) -> Dict[str, Any]:
        ...
        return {"status_filter": self.current_filter, "allowed_statuses": self.STATUS_MAPPINGS[self.current_filter]}
