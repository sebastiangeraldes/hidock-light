...
"""
Calendar Search Widget for HiDock Light
...
class CalendarSearchWidget(ctk.CTkFrame):
    ...
    def _create_widgets(self):
        ...
        # Search label
        self.search_label = ctk.CTkLabel(self, text="🔍 Search Meetings:", font=ctk.CTkFont(size=12, weight="bold"))
        self.search_label.grid(row=0, column=0, padx=(5, 10), pady=5, sticky="w")

        # Search entry with placeholder
        self.search_entry = ctk.CTkEntry(self, placeholder_text="Search by meeting subject...", width=300, height=32)
        ...
        # Clear button
        self.clear_button = ctk.CTkButton(self, text="Clear", width=60, height=32, command=self._clear_search)
        self.clear_button.grid(row=0, column=2, padx=(5, 0), pady=5)

        # Status label (shows active filters and result count)
        self.status_label = ctk.CTkLabel(self, text="", font=ctk.CTkFont(size=10), text_color="gray")
    ...
    def _perform_search(self, search_text: str):
        ...
        try:
            ...
            if search_text:
                self.current_filters = {"subject": search_text, "search_type": "subject"}
            else:
                self.current_filters = {}
