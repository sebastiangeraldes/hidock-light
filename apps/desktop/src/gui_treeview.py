# gui_treeview.py
"""
TreeView Mixin for the HiDock Light GUI.
"""
    ...
    def _sort_files_data(self, files_data, col, reverse):
        ...
        def sort_key(item):
            if col == "size":
                ...
                # Meeting column data is stored in 'meeting_display_text' field
                val = item.get("meeting_display_text", "")
                return str(val).lower() if isinstance(val, str) else str(val)
            elif col == "status":
                # Status column data is stored in 'gui_status' field
                val = item.get("gui_status", "")
                return str(val).lower() if isinstance(val, str) else str(val)
            else:
                # Fallback for other columns like 'name'
                val = item.get(col)
                if val is None:
                    return ""
    ...
    def apply_status_filter(self, filter_criteria: Dict[str, Any]):
        ...
        Args:
            filter_criteria: Dictionary containing filter parameters
                ...
                - allowed_statuses: List of status strings that should be shown, or None for all
        """
        try:
