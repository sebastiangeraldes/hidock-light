...
class ExtendedJensenProtocol(HiDockJensen):
    ...
    def enable_extended_protocol(self) -> bool:
        ...
        try:
            ...
            response = self._send_and_receive(
                ExtendedJensenCommands.CMD_PROTOCOL_VERSION_EXT, b"ENABLE_EXT", timeout_ms=10000
            )
            ...
            if response and len(response) >= 4:
                ...
                logger.error("ExtendedJensen", "enable_extended_protocol", "Invalid response from device")
                return False

        except Exception as e:
            logger.error("ExtendedJensen", "enable_extended_protocol", f"Failed to enable extended protocol: {e}")
            return False
    ...
    def get_hardware_info(self) -> Optional[HardwareInfo]:
        ...
        if not self._extended_protocol_enabled:
            logger.error("ExtendedJensen", "get_hardware_info", "Extended protocol not enabled")
            return None

        try:
            response = self._send_and_receive(ExtendedJensenCommands.CMD_GET_HARDWARE_INFO, timeout_ms=5000)

            if not response or len(response) < 64:
                logger.error("ExtendedJensen", "get_hardware_info", "Invalid or insufficient response data")
                return None
            ...
            logger.info(
                "ExtendedJensen", "get_hardware_info", f"Hardware info retrieved: CPU={cpu_freq}MHz, DSP={dsp_freq}MHz"
            )

            return hardware_info

        except Exception as e:
            logger.error("ExtendedJensen", "get_hardware_info", f"Failed to get hardware info: {e}")
            return None
    ...
    def direct_memory_read(self, address: int, size: int) -> Optional[bytes]:
        ...
        """
        if not self._extended_protocol_enabled:
            logger.error("ExtendedJensen", "direct_memory_read", "Extended protocol not enabled")
            return None

        if size > 4096 or size <= 0:
            logger.error("ExtendedJensen", "direct_memory_read", f"Invalid size: {size} (must be 1-4096)")
            return None
        ...
        try:
            ...
            response = self._send_and_receive(ExtendedJensenCommands.CMD_DIRECT_MEMORY_READ, payload, timeout_ms=10000)

            if not response:
                logger.error("ExtendedJensen", "direct_memory_read", "No response from device")
                return None

            if len(response) != size:
                logger.warning(
                    "ExtendedJensen", "direct_memory_read", f"Size mismatch: requested {size}, got {len(response)}"
                )

            logger.debug("ExtendedJensen", "direct_memory_read", f"Read {len(response)} bytes from 0x{address:08X}")

            return response

        except Exception as e:
            logger.error("ExtendedJensen", "direct_memory_read", f"Memory read failed: {e}")
            return None
    ...
    def direct_memory_write(self, address: int, data: bytes) -> bool:
        ...
        """
        if not self._extended_protocol_enabled:
            logger.error("ExtendedJensen", "direct_memory_write", "Extended protocol not enabled")
            return False

        if len(data) > 4096 or len(data) == 0:
            logger.error("ExtendedJensen", "direct_memory_write", f"Invalid data size: {len(data)} (must be 1-4096)")
            return False
        ...
        for start, end in PROTECTED_REGIONS:
            if start <= address < end or start < address + len(data) <= end:
                logger.error(
                    "ExtendedJensen", "direct_memory_write", f"Write to protected region 0x{address:08X} blocked"
                )
                return False
        ...
        try:
            ...
            response = self._send_and_receive(ExtendedJensenCommands.CMD_DIRECT_MEMORY_WRITE, payload, timeout_ms=10000)
            ...
            if response and len(response) >= 4:
                ...
                if success:
                    logger.info("ExtendedJensen", "direct_memory_write", f"Wrote {len(data)} bytes to 0x{address:08X}")
                else:
                    logger.error(
                        "ExtendedJensen", "direct_memory_write", f"Write failed, device error code: 0x{result:08X}"
                    )

                return success
            else:
                logger.error("ExtendedJensen", "direct_memory_write", "Invalid response from device")
                return False

        except Exception as e:
            logger.error("ExtendedJensen", "direct_memory_write", f"Memory write failed: {e}")
            return False
    ...
    def get_performance_metrics(self) -> Optional[PerformanceMetrics]:
        ...
        """
        if not self._extended_protocol_enabled:
            logger.error("ExtendedJensen", "get_performance_metrics", "Extended protocol not enabled")
            return None

        try:
            response = self._send_and_receive(ExtendedJensenCommands.CMD_PERFORMANCE_MONITORING, timeout_ms=3000)

            if not response or len(response) < 32:
                logger.error("ExtendedJensen", "get_performance_metrics", "Invalid response data")
                return None
            ...
            logger.error("ExtendedJensen", "get_performance_metrics", f"Failed to get performance metrics: {e}")
            return None
    ...
    def storage_raw_read(self, block_address: int, block_count: int) -> Optional[bytes]:
        ...
        """
        if not self._extended_protocol_enabled:
            logger.error("ExtendedJensen", "storage_raw_read", "Extended protocol not enabled")
            return None

        if block_count > 64 or block_count <= 0:
            logger.error("ExtendedJensen", "storage_raw_read", f"Invalid block count: {block_count} (must be 1-64)")
            return None
        ...
        try:
            ...
            if not response:
                logger.error("ExtendedJensen", "storage_raw_read", "No response from device")
                return None
            ...
            logger.info("ExtendedJensen", "storage_raw_read", f"Read {block_count} blocks from address {block_address}")

            return response

        except Exception as e:
            logger.error("ExtendedJensen", "storage_raw_read", f"Storage read failed: {e}")
            return None
    ...
    def audio_register_read(self, register_id: int) -> Optional[int]:
        ...
        """
        if not self._extended_protocol_enabled:
            logger.error("ExtendedJensen", "audio_register_read", "Extended protocol not enabled")
            return None

        if register_id > 31:
            logger.error("ExtendedJensen", "audio_register_read", f"Invalid register ID: {register_id}")
            return None
        ...
        try:
            ...
            response = self._send_and_receive(
                ExtendedJensenCommands.CMD_AUDIO_REGISTER_ACCESS, payload, timeout_ms=3000
            )

            if response and len(response) >= 4:
                register_value = struct.unpack(">I", response[:4])[0]
                logger.debug("ExtendedJensen", "audio_register_read", f"Register {register_id}: 0x{register_value:08X}")
                return register_value
            else:
                logger.error("ExtendedJensen", "audio_register_read", "Invalid response from device")
                return None

        except Exception as e:
            logger.error("ExtendedJensen", "audio_register_read", f"Register read failed: {e}")
            return None
    ...
    def clear_performance_history(self):
        ...
        logger.info("ExtendedJensen", "clear_performance_history", "Performance history cleared")
