    ...
    try:
        ...
        if not lib_path:
            ...
            if not backend_instance:
                logger.error(
                    "Discovery", "initialize_usb_backend", f"libusb backend failed from system paths on {system}"
                )
                return None
        else:
            ...
            if not backend_instance:
                logger.error(
                    "Discovery", "initialize_usb_backend", f"Failed to initialize backend with library: {lib_path}"
                )
                return None

        logger.info("Discovery", "initialize_usb_backend", f"USB backend initialized successfully on {system}")
...
class JensenCommandDiscovery:
    ...
    def test_single_command(self, command_id: int, test_payload: bytes = b"") -> CommandResult:
        ...
        logger.debug(
            "Discovery", "test_single_command", f"Testing CMD {command_id} with {len(test_payload)} byte payload"
        )
        ...
        try:
            ...
            response = self.device._send_and_receive(command_id, test_payload, timeout_ms=self.test_timeout)
            ...
            if response:
                ...
                logger.debug(
                    "Discovery", "test_single_command", f"[?] CMD {command_id}: NO RESPONSE ({response_time:.1f}ms)"
                )
            ...
            logger.debug("Discovery", "test_single_command", f"[FAIL] CMD {command_id}: {status.upper()} - {error_msg}")
    ...
    def discover_commands_in_range(self, start_cmd: int, end_cmd: int) -> List[CommandResult]:
        ...
        for cmd_id in range(start_cmd, end_cmd + 1):
            ...
            if cmd_id % 10 == 0 or result.status == "supported":
                ...
                logger.info("Discovery", "discover_commands_in_range", f"Progress: {progress:.1f}% (CMD {cmd_id})")
    ...
    def test_missing_commands(self) -> List[CommandResult]:
        ...
        logger.info("Discovery", "test_missing_commands", "Testing missing commands: 10, 14, 15")
        ...
        for cmd_id in missing_commands:
            ...
            if result.status == "supported":
                logger.info(
                    "Discovery", "test_missing_commands", f"[FOUND] Missing command {cmd_id}! Testing with payloads..."
                )
    ...
    def validate_known_commands(self) -> List[CommandResult]:
        ...
        logger.info("Discovery", "validate_known_commands", "Validating known commands 1-20")
        ...
        for cmd_id in known_commands:
            ...
            if result.status != "supported" and cmd_id in self.known_safe_commands:
                logger.warning(
                    "Discovery", "validate_known_commands", f"⚠️ Known command {cmd_id} not responding as expected!"
                )
    ...
    def comprehensive_discovery(self, max_command_id: int = 50) -> DiscoverySession:
        ...
        logger.info(
            "Discovery", "comprehensive_discovery", f"Starting comprehensive command discovery (1-{max_command_id})"
        )
    ...
    def save_discovery_results(self, session: DiscoverySession, filename: str = None):
        ...
        try:
            ...
            logger.info("Discovery", "save_discovery_results", f"Discovery results saved to {filename}")
        except Exception as e:
            logger.error("Discovery", "save_discovery_results", f"Failed to save results: {e}")
    ...
    def print_discovery_summary(self, session: DiscoverySession):
        ...
        print(
            f"🎯 Commands Tested: {session.total_commands_tested} "
        ...
        if session.supported_commands:
            ...
            for cmd_id in session.supported_commands:
                ...
                print(
                    f"  CMD {cmd_id:2d}: {result.response_length:4d} bytes "
...
def main():
    ...
    parser.add_argument(
        "--safe-mode", action="store_true", default=True, help="Enable safe mode (skip potentially dangerous commands)"
    )
    parser.add_argument("--full-scan", action="store_true", help="Disable safe mode and test all commands")
    parser.add_argument("--range", type=str, default="1-50", help='Command range to test (e.g., "1-50", "10-15")')
    parser.add_argument("--missing-only", action="store_true", help="Test only missing commands 10, 14, 15")
