...
"""
Calendar Filter Engine for HiDock Light
...
class CalendarFilterEngine:
        ...
        try:
            ...
            if "subject" in filters and filters["subject"]:
                filtered_files = self.filter_by_subject(
                    filtered_files, filters["subject"], fuzzy=filters.get("fuzzy_match", True)
                )

            # Apply participant filter (future enhancement)
            if "participant" in filters and filters["participant"]:
                filtered_files = self.filter_by_participant(filtered_files, filters["participant"])
            ...
            if "date_range" in filters and filters["date_range"]:
                ...
                if isinstance(date_range, dict) and "start" in date_range and "end" in date_range:
                    filtered_files = self.filter_by_date_range(filtered_files, date_range["start"], date_range["end"])

            # Apply meeting type filter (future enhancement)
            if "meeting_types" in filters and filters["meeting_types"]:
                filtered_files = self.filter_by_meeting_type(filtered_files, filters["meeting_types"])

            logger.debug(
                "CalendarFilter", "apply_filters", f"Filtered {len(files_data)} files to {len(filtered_files)} results"
            )
    ...
    def filter_by_subject(
        self, files_data: List[Dict[str, Any]], search_term: str, fuzzy: bool = True
    ) -> List[Dict[str, Any]]:
        """
    ...
    def filter_by_date_range(
        self, files_data: List[Dict[str, Any]], start_date: datetime, end_date: datetime
    ) -> List[Dict[str, Any]]:
        """
    ...
    def filter_by_meeting_type(
        self, files_data: List[Dict[str, Any]], meeting_types: List[str]
    ) -> List[Dict[str, Any]]:
        """
    ...
    def get_filter_suggestions(self, files_data: List[Dict[str, Any]], filter_type: str) -> List[str]:
        ...
        try:
            ...
            logger.error("CalendarFilter", "get_filter_suggestions", f"Error generating suggestions: {e}")
            return []
