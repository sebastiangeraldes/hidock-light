...
"""
Unified Filter Widget for HiDock Light
...
class UnifiedFilterWidget(ctk.CTkFrame):
    ...
    def __init__(
        self, parent, calendar_callback: Optional[Callable] = None, status_callback: Optional[Callable] = None, **kwargs
    ):
    ...
    def _create_unified_interface(self):
        ...
        # Search icon + label
        self.search_icon_label = ctk.CTkLabel(self, text="🔍", font=ctk.CTkFont(size=16))
        self.search_icon_label.grid(row=0, column=0, padx=(0, 5), pady=8, sticky="w")

        # Search entry (compact, no placeholder needed)
        self.search_entry = ctk.CTkEntry(self, placeholder_text="Search meetings...", width=240, height=32)
        ...
        # === SEPARATOR ===
        self.separator = ctk.CTkLabel(self, text="│", font=ctk.CTkFont(size=20), text_color="gray50")
        self.separator.grid(row=0, column=2, padx=15, pady=8)

        # === RIGHT SIDE: Status Filter ===
        # Status icon + label
        self.status_icon_label = ctk.CTkLabel(self, text="📁", font=ctk.CTkFont(size=16))
        self.status_icon_label.grid(row=0, column=3, padx=(0, 5), pady=8, sticky="w")

        # Status dropdown (compact)
        self.status_dropdown = ctk.CTkOptionMenu(
            self, width=130, height=32, values=list(self.STATUS_FILTERS.values()), command=self._on_status_changed
        )
        self.status_dropdown.set(self.STATUS_FILTERS["all"])
        self.status_dropdown.grid(row=0, column=4, padx=0, pady=8)

        # === UNIFIED STATUS DISPLAY ===
        self.status_display = ctk.CTkLabel(self, text="", font=ctk.CTkFont(size=11), text_color="gray60")
    ...
    def _perform_search(self, search_text: str):
        ...
        try:
            ...
            if search_text:
                self.current_calendar_filters = {"subject": search_text, "search_type": "subject"}
            else:
                self.current_calendar_filters = {}
    ...
    def _on_status_changed(self, selected_display_name: str):
        ...
        try:
            ...
            filter_criteria = {"status_filter": filter_key, "allowed_statuses": self.STATUS_MAPPINGS[filter_key]}
