...
HiDock Light application. It adds advanced hardware access capabilities to the GUI
...
class ExtendedFeaturesManager:
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            if not self.extended_protocol.is_connected():
                logger.error("ExtendedFeatures", "initialize", "Device not connected, cannot enable extended features")
                return False

            # Enable extended protocol
            if not self.extended_protocol.enable_extended_protocol():
                logger.error("ExtendedFeatures", "initialize", "Failed to enable extended protocol")
                return False

            # Get hardware information
            self.hardware_info = self.extended_protocol.get_hardware_info()
            if not self.hardware_info:
                logger.warning("ExtendedFeatures", "initialize", "Could not retrieve hardware info, continuing anyway")

            self.is_initialized = True
            logger.info("ExtendedFeatures", "initialize", "Extended features initialized successfully")

            return True

        except Exception as e:
            logger.error("ExtendedFeatures", "initialize", f"Initialization failed: {e}")
            return False
    ...
    def get_hardware_info(self) -> Optional[Dict]:
        ...
        if not self.is_initialized:
            logger.error("ExtendedFeatures", "get_hardware_info", "Extended features not initialized")
            return None
    ...
    def get_performance_metrics(self) -> Optional[Dict]:
        ...
        """
        if not self.is_initialized:
            logger.error("ExtendedFeatures", "get_performance_metrics", "Extended features not initialized")
            return None
    ...
    def read_memory(self, address: int, size: int) -> Optional[bytes]:
        ...
        """
        if not self.is_initialized:
            logger.error("ExtendedFeatures", "read_memory", "Extended features not initialized")
            return None

        # Safety check for reasonable memory ranges
        if address < 0x20000000 or address > 0xA0000000:
            logger.warning(
                "ExtendedFeatures", "read_memory", f"Reading from potentially unsafe address: 0x{address:08X}"
            )
    ...
    def read_storage_blocks(self, block_address: int, block_count: int) -> Optional[bytes]:
        ...
        """
        if not self.is_initialized:
            logger.error("ExtendedFeatures", "read_storage_blocks", "Extended features not initialized")
            return None
    ...
    def start_performance_monitoring(self, callback: Optional[Callable] = None):
        ...
        """
        if not self.is_initialized:
            logger.error("ExtendedFeatures", "start_performance_monitoring", "Extended features not initialized")
            return

        if self.monitoring_active:
            logger.warning("ExtendedFeatures", "start_performance_monitoring", "Performance monitoring already active")
            return
        ...
        logger.info("ExtendedFeatures", "start_performance_monitoring", "Performance monitoring started")
    ...
    def stop_performance_monitoring(self):
        ...
        logger.info("ExtendedFeatures", "stop_performance_monitoring", "Performance monitoring stopped")
    ...
    def _performance_monitoring_loop(self):
        ...
        logger.info("ExtendedFeatures", "_performance_monitoring_loop", "Performance monitoring thread started")
                ...
                if metrics:
                    ...
                    for callback in self.monitoring_callbacks:
                        try:
                            ...
                            logger.error("ExtendedFeatures", "_performance_monitoring_loop", f"Callback error: {e}")

                # Wait for next monitoring cycle
                time.sleep(self.monitoring_interval)

            except Exception as e:
                logger.error("ExtendedFeatures", "_performance_monitoring_loop", f"Monitoring error: {e}")
                time.sleep(self.monitoring_interval)  # Continue monitoring despite errors

        logger.info("ExtendedFeatures", "_performance_monitoring_loop", "Performance monitoring thread stopped")
    ...
    def _check_performance_alerts(self, metrics: PerformanceMetrics):
        ...
        if alerts:
            logger.warning("ExtendedFeatures", "_check_performance_alerts", f"Performance alerts: {', '.join(alerts)}")
...
class ExtendedFeaturesGUIIntegration:
    ...
    def create_hardware_info_display(parent, hardware_info: HardwareInfo) -> dict:
        ...
        # Title
        title_label = ctk.CTkLabel(info_frame, text="Hardware Information", font=ctk.CTkFont(size=16, weight="bold"))
...
Zephyr: {hardware_info.zephyr_version}"""
        ...
        specs_label = ctk.CTkLabel(info_frame, text=specs_text, justify="left", font=ctk.CTkFont(family="monospace"))
    ...
    def create_performance_monitor_display(parent) -> dict:
        ...
        # Title
        title_label = ctk.CTkLabel(perf_frame, text="Performance Monitor", font=ctk.CTkFont(size=16, weight="bold"))
        title_label.pack(pady=(10, 5))
        widgets["title"] = title_label
...
def test_extended_features(usb_backend):
    ...
    if not ext_manager.initialize():
        logger.error("ExtendedFeatures", "test_extended_features", "Failed to initialize extended features")
        return False
    ...
    if hardware_info:
        logger.info(
            ...
            f"Hardware Info: CPU={hardware_info['cpu_frequency']}MHz, " f"DSP={hardware_info['dsp_frequency']}MHz",
        )
    ...
    if performance:
        logger.info(
            "ExtendedFeatures",
            "test_extended_features",
            f"Performance: CPU={performance['cpu_usage']:.1f}%, " f"Temp={performance['system_temperature']:.1f}°C",
        )

    # Test memory read (safe region)
    memory_data = ext_manager.read_memory(0x20000000, 256)  # Read from SRAM
    if memory_data:
        logger.info("ExtendedFeatures", "test_extended_features", f"Memory read successful: {len(memory_data)} bytes")

    # Test storage read
    storage_data = ext_manager.read_storage_blocks(0, 1)  # Read first block
    if storage_data:
        logger.info("ExtendedFeatures", "test_extended_features", f"Storage read successful: {len(storage_data)} bytes")

    # Get capabilities summary
    capabilities = ext_manager.get_device_capabilities_summary()
    logger.info("ExtendedFeatures", "test_extended_features", f"Device capabilities: {capabilities}")
