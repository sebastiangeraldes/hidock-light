...
"""
Simple Outlook Calendar Integration for HiDock Light
...
class SimpleOutlookIntegration:
    ...
    def _create_meeting_from_ics_event(self, event: Dict[str, str], target_date: datetime) -> Optional[SimpleMeeting]:
        ...
        try:
            ...
            return SimpleMeeting(
                subject=subject, start_time=start_time, end_time=end_time, organizer=organizer, location=location
            )
        ...
        try:
            ...
            for date_format in filter_formats:
                try:
                    ...
                    if count == 2147483647:
                        logger.warning("Date filter returned max int, trying without recurrences")
    ...
    def _get_meetings_day_by_day(self, start_date: datetime, end_date: datetime) -> List[SimpleMeeting]:
        ...
        logger.info(
            f"Found {len(all_meetings)} total meetings via day-by-day for range {start_date.date()} to {end_date.date()}"
        )
        return all_meetings

    def find_meeting_for_recording(
        self, recording_time: datetime, tolerance_minutes: int = 15
    ) -> Optional[SimpleMeeting]:
